...
class SphereTest : public testing::Test{
  protected:
    static shapes::Point randPoint(double &x,
 		  double &y, double &z){
      x = rand()%10;
      y = rand()%10;
      z = rand()%10;
      return shapes::Point(x, y, z);
    }
...
TEST_F(SphereTest,TestGetPrivateAttrs){
  ...
  shapes::Point init = randPoint(x, y, z);
  shapes::Sphere S(init, radius);
...
TEST_F(SphereTest, TestLessTrue){
  double r1 = rand()%10+1;
  double x1,y1,z1;
  shapes::Point init = randPoint(x1,y1,z1);
  shapes::Sphere Small(init, r1);
  double r2 = r1*(rand()%3+2);
  double x2,y2,z2;
  shapes::Point init2 = randPoint(x2,y2,z2);
  shapes::Sphere Large(init2, r2);   
...
TEST_F(SphereTest, TestLargestEqual){
  ...
  std::vector <shapes::Sphere> spheres(size, S);
  shapes::Sphere Actual = largest(spheres);
...
TEST_F(SphereTest, TestLargestNotEqual){
  ...
  int size = static_cast<int>(rand()%10+20);
  int l = size-1;
  std::vector <shapes::Sphere> spheres(size);
  ...
  for (int i=0; i<size; i++){
    shapes::Point center = randPoint(x[i], y[i], z[i]);
    radius[i] = r*(i+1);
    spheres[i] = shapes::Sphere(center, radius[i]);
  }
